...
#Added Tier tag to each subnet type
...
resource "aws_subnet" "public" {
  ...
  tags = "${merge(map("Tier", "Public"), map("Name", format("%s-${var.public_subnet_suffix}-%s", var.name, element(data.aws_availability_zones.all.names, count.index))), var.tags, var.public_subnet_tags, var.eks_binds_public_subnet ? var.public_subnet_kubernetes_lb_tag : null )}"
}
...
resource "aws_subnet" "private" {
  ...
  tags = "${merge(map("Tier", "Private"), map("Name", format("%s-${var.private_subnet_suffix}-%s", var.name, element(data.aws_availability_zones.all.names, count.index))), var.tags, var.private_subnet_tags, var.eks_binds_private_subnet ? var.private_subnet_kubernetes_lb_tag : null)}"
}
