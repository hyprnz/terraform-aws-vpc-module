...
resource "aws_route" "private_transit_gateway" {
  ...
  route_table_id = element(
    ...
    floor(count.index / length(aws_route_table.private.*.id)),
  )
  destination_cidr_block = element(
    concat(var.transit_gateway_destination_cidrs, ["default"]),
    count.index % length(concat(var.transit_gateway_destination_cidrs, [""])),
  )
  transit_gateway_id = var.transit_gateway_id
